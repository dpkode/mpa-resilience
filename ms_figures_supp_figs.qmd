...
# A total of 5 (2023/12/20) simulations were run for each paramter set
...
out_fs <- df_add_fs_out(
  input_df = sim_res_out,
  spec_fleps_2_fs = spec_fleps_2_fs,
  F_maxes_yield = F_brf_maxes_yield,
...
bio_yield_sel_df <- bio_yield_df %>%
  ...
  dplyr::summarize(annual_biomass = sum(biomass_at_age),
                   ...
                   rel_annual_biomass = sum(biomass_at_age) / med_biomass,
                   rel_annual_yield = sum(yield) / med_yield) %>%
  dplyr::ungroup()
...
yld_ts_plt <- bio_yield_sel_df %>%
  ...
  ggplot(data = ., aes(x = year, y = rel_annual_yield)) +
  facet_grid(reserve_frac ~ f_fmsy,
             labeller = labeller(f_fmsy = label_both,
                                 reserve_frac = label_both)) +
  geom_ribbon(aes(
...
summary_stats_df_plot <- summary_stats_df[[2]] %>%
  dplyr::filter(reduce(map(
...
summary_stats_df_plot_yld <- summary_stats_df[[2]] %>%
  dplyr::filter(reduce(map(
    frac_reserves, near, x = as.numeric(reserve_frac), tol = 1e-4
  ), `|`)) %>%
  dplyr::filter(reduce(map(
...
ggsave(filename = "output/tmp_figs/fig5-biomass-timeseries-text.png", plot = biom_ts_plt,
       height = 8, width = 14)
ggsave(filename = "output/tmp_figs/fig6-yield-timeseries-text.png", plot = yld_ts_plt,
       height = 8, width = 14)

ggsave(filename = "output/tmp_figs/fig5-biomass-timeseries-text.svg", plot = biom_ts_plt,
       height = 8, width = 14)
ggsave(filename = "output/tmp_figs/fig6-yield-timeseries-text.svg", plot = yld_ts_plt,
       height = 8, width = 14)
```
